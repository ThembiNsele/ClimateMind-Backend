...
# value_nodes = {"increase in physical violence": list()}
...
for value_key, cyto_elements in value_nodes.items():
    ...
    # subtree.remove_nodes_from(nodes_to_remove)

    N = nx.nx_agraph.to_agraph(subtree)

    N.edge_attr.update(directed=True)
    N.layout(prog='dot', args='-Gratio=compress -Gsize="30, 30" -Grankdir=BT -Gnodesep=0.01 -Nmargin=0.01')
    ...
    # Extract x y positions using graphviz dot algo.
